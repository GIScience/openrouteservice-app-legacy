...
//-----------------------------------------------------------------------------
      ...
      function make_large_layer(data_url, style, zoom) {
...
//          });
	  ...
	  var strategy = new ZoomLimitedBBOXStrategy(zoom);
          var layer = new OpenLayers.Layer.Vector(styleMap.fillColor, {
              strategies: [strategy],
          ...
          layer.events.register("featuresadded", layer, function(){
        	  make_features_added_closure(layer);
          });
          return layer;
      }

      function make_layer(data_url, style) {
	  return make_large_layer(data_url, style, 8);
      }
