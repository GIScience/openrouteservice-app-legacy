...
OpenRouteService.convert = ( function() {'use strict';
		var namespaces = OpenRouteService.namespaces;
		var convert = this;
		var Util = OpenRouteService.Util;
				...
				distance2hr : function(distance, uom) {
					...
					if (uom == OpenRouteService.List.distanceUnitsRoute[0]) {
						if (distance >= 1000) {
							...
							distance = OpenRouteService.Util.round(distance);
						} else {
							uom = 'M';
						}
						distance = OpenRouteService.Util.round(distance);
			...
			xls2html : {
				...
				poi2div : function(poi, poisNearRoute, distanceUnit) {
					...
					if (!poisNearRoute) {
						...
						var dist = OpenRouteService.Util.convertDistToDist(poiDist, distanceUnitSrc, distanceUnit);
