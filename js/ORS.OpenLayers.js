( function() {
	...
	OpenLayers.MarkerEm = OpenLayers.Class(OpenLayers.Marker, {
		 ...
		 */
		initialize : function(lonlat, deEmIcon, emIcon, id) {
			this.lonlat = lonlat;
			this.id = id; 
			var newIcon = (deEmIcon) ? deEmIcon.clone() : OpenLayers.Marker.defaultIcon();
		...
		/**
		 * Method: emphasize
		 * Switch displayed Icon to emIcon - the emphasized verision.
		 */
		emphasize : function() {
		...
		/**
		 * Method: emphasize
		 * Switch displayed Icon to deEmIcon - the deemphasized verision.
		 */
		...
		toggleEmph : function() {
			if (this.icon.url == this.deEmIcon.url) {
				this.emphasize();
			} else {
				this.deemphasize();
			}
