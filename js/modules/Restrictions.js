	...
	*/
	function filterByAllAttributes(restriction){
		...
		for (var tag in restriction.tags){
			...
			switch (tag) {
				case "maxlength":
				try {featureLength = parseFloat(restriction.tags[tag]);}
				catch(e) {break;} //tag exists but is not a number so just leave it in there, might still be useful to the user
				if (featureLength < vehicleLength) break;
				delete restriction.tags[tag];
				break;
				case "maxwidth":
				try {featureWidth = parseFloat(restriction.tags[tag]);}
				catch(e) {break;} //tag exists but is not a number so just leave it in there, might still be useful to the user
				if (featureWidth < vehicleWidth) break;
				delete restriction.tags[tag];
				break;
				case "maxheight":
				try {featureHeight = parseFloat(restriction.tags[tag]);}
				catch(e) {break;} //tag exists but is not a number so just leave it in there, might still be useful to the user
				if (featureHeight < vehicleHeight) break;
				delete restriction.tags[tag];
				break;
				case "maxweight":
				try {featureWeight = parseFloat(restriction.tags[tag]);}
				catch(e) {break;} //tag exists but is not a number so just leave it in there, might still be useful to the user
				if (featureWeight < vehicleWeight) break;
				delete restriction.tags[tag];
				break;
				case "maxaxleload":
				try {featureAxleload = parseFloat(restriction.tags[tag]);}
				catch(e) {break;} //tag exists but is not a number so just leave it in there, might still be useful to the user
